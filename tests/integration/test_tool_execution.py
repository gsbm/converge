...
class _AgentWithToolDecision(Agent):
    ...
    def decide(self, messages, tasks, **kwargs):
        if messages:
            return [InvokeTool(tool_name="echo", params={"called": True})]
        return []


class _AgentReActStyle(Agent):
    """First call: InvokeTool. When tool_observations present: no more tools (loop terminates)."""

    def decide(self, messages, tasks, tool_observations=None, **kwargs):
        if tool_observations:
            return []
        if messages or tasks:
            return [InvokeTool(tool_name="echo", params={"step": 1})]
        return []
...
async def test_tool_loop_feeds_back_observations():
    """ReAct loop: agent emits InvokeTool, runtime runs tool and re-calls decide with tool_observations; agent then returns []."""
    registry = LocalTransportRegistry()
    registry.clear()
    ...
    identity = Identity.generate()
    agent = _AgentReActStyle(identity)
    transport = LocalTransport(agent.id)
    pool_manager = PoolManager(store=MemoryStore())
    task_manager = TaskManager(store=MemoryStore())
